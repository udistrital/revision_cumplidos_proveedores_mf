...
export class ModalComentariosSoporteComponent implements OnInit {
  ...
  ObtenerUltimoCambioEstado(){
    ...
    this.cumplidos_provedore_crud_service.get(`/cambio_estado_cumplido/?query=Activo:true,CumplidoProveedorId.Id:${this.data.CumplidoProveedorId}&sortby=FechaCreacion&order=desc`).subscribe({
      next:(response)=>{
        this.isLoading = false;
