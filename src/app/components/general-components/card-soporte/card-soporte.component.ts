...
export class CardSoporteComponent {
  ...
  async eliminarSoporte() {
    const confirm = await this.aletManagerService.alertConfirm(
    ...
    if (confirm.isConfirmed) {
      ...
      try {
        this.cumplidosMidServices
          .delete(`/solicitud-pago/soportes`, this.soporte.Documento)
          .subscribe({
            next: (res: any) => {
        ...
        if (confirmed.isConfirmed) {
          ...
          try {
            ...
            this.cumplidos_provedore_crud_service
              .post('/comentario_soporte', comentario)
              .subscribe((response) => {
                this.aletManagerService.showSuccessAlert(
                  'Comentario Guardado',
                  'Se ha guardado el comentario en el documento'
                );
              });
          } catch (error) {
            this.aletManagerService.showCancelAlert(
              'Cancelado',
              'No se ha guardo ningun comentario'
            );
          }
        }
      })
